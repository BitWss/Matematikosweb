...
SHARED_CSS = """
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .section-header:hover { color: #2980b9; border-color: #2980b9; }
    .section-header .arrow { transition: transform 0.3s; font-size: 0.75rem; opacity: 0.5; color: #95a5a6; }
    .section-header.open .arrow { transform: rotate(180deg); }
    .section-body { display: none; padding: 16px 0; gap: 16px; flex-direction: column; }
    .section-body.open { display: flex; }
    .pdf-item { border: none; border-radius: 0; overflow: visible; }
    .pdf-label {
      background: transparent;
      border: none;
      padding: 10px 0;
    ...
    .pdf-arrow { font-size: 0.7rem; opacity: 0.5; transition: transform 0.3s; }
    .pdf-arrow.open { transform: rotate(180deg); }
    .pdf-collapse { display: none; }
    .pdf-collapse.open { display: block; }
...
def build_class_page(class_folder, class_title, themes):
    ...
    for theme_folder, pdfs in themes:
        ...
        for pdf in pdfs:
            ...
            pdf_items += f"""
      <div class="pdf-item">
        <div class="pdf-label" onclick="togglePdf(this)">
          <span>{label}</span>
          <div class="pdf-label-actions">
            <a href="{path}" target="_blank" onclick="event.stopPropagation()">Atsisiųsti</a>
...
{sections_html}
  <script>
    ...
    function togglePdf(label) {{
      const collapse = label.nextElementSibling;
      const arrow    = label.querySelector('.pdf-arrow');
...
def build_index(classes):
    ...
    for class_folder, class_title, slug, theme_count in classes:
      ...
      <div class="card-meta">{theme_count} {"tema" if theme_count == 1 else "temos"}</div>
    </a>
"""
...
def build():
    if not os.path.isdir(PDF_ROOT):
        ...
        print(
            f"Sukurtas aplankas '{PDF_ROOT}/'. Pridėkite klasių aplankus su temų poaplankiais."
        )
        return

    class_folders = sorted(
        [f for f in os.listdir(PDF_ROOT) if os.path.isdir(os.path.join(PDF_ROOT, f))]
    )
    ...
    for class_folder in class_folders:
        ...
        theme_folders = sorted(
            [
                t
                for t in os.listdir(class_path)
        ...
        for theme_folder in theme_folders:
            ...
            pdfs = sorted(
                [f for f in os.listdir(theme_path) if f.lower().endswith(".pdf")]
            )
